...
;(function(window) {
	...
	var SLIDESHOW_MOVE_DELAY_MS = 8000;
	...
	function CircleSlideshow(el, options) {
		console.log("CircleSlideshow called");
		if (!el) {
			...
			// items
			var items = [].slice.call(el.querySelectorAll('.slide'));
			// total items
	...
	CircleSlideshow.prototype._openContent = function(item) {
		...
		var self = this,
			...
			messageEl = contentEl.querySelector('.slide__message');
		...
		// - message
		dynamics.css(messageEl, {translateY : 400, opacity: 0});
		// - buy button
		...
		// animate the message element in
		dynamics.animate(messageEl, 
			{
				translateY : 0, opacity : 1
			}, 
			{
	...
	CircleSlideshow.prototype._closeContent = function() {
		...
		// animate the message element out
		dynamics.stop(messageEl);
		dynamics.animate(messageEl, 
			{
